...
template <typename w_data_type, typename a_data_type, typename packet_type, bool use_relu, int batch_size, int in_channels, int out_channels, int in_height, int in_width, int padding>
int conv2D_3x3_IM_base(hls::stream<packet_type> &input, hls::stream<packet_type> &output, const mat3<w_data_type> kernel[in_channels][out_channels])
{
...
#pragma HLS INTERFACE s_axilite port = return
    ...
    a_data_type out_buffer[out_channels][out_height][out_width];

    shift_mat3<a_data_type, in_width + padding * 2> shift_reg;
        ...
        in_y_loop:
            ...
            in_x_loop:
                for (int x = -padding; x < in_width + padding; x++)
                {
                    ...
                    a_data_type in_data = a_data_type(0.0f);
                    if (x >= 0 && x < in_width && y >= 0 && y < in_height)
                    {
                        packet_type in_packet;
                        input.read(in_packet);
                        in_data = a_data_type(in_packet.data);
                    }
                    // if (in_packet.last == 1)
                    //     last_was_read = true;

                    shift_reg.shift_down(in_data);
                    mat3<a_data_type> data = shift_reg.getMat();

                main_out_channel_loop:
                    for (int out_channel = 0; out_channel < out_channels; out_channel++)
                    {
                        a_data_type conv = data.template mul_v2<a_data_type, w_data_type>(kernel[in_channel][out_channel]);
    ...
    out_channel_loop:
        ...
        out_y_loop:
            ...
            out_x_loop:
                ...
                {
                    a_data_type out_data = out_buffer[out_channel][y][x];

                    if (use_relu)
                        if (out_data < a_data_type(0.0f))
                            out_data = a_data_type(0.0f);
