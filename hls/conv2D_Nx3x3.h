...
template <typename data_type, typename packet_type, int M, int N, int height, int width>
int conv2D_MxNx3x3(hls::stream<packet_type> &input, hls::stream<packet_type> &output, float weights[M * N * 3 * 3])
{
...
#pragma HLS INTERFACE s_axilite port = return
    ...
    {
        for (int in_channel = 0; in_channel < N; in_channel++)
        {
        init_kernel_y_loop:
            for (int y = 0; y < 3; y++)
            {
#pragma HLS UNROLL
            init_kernel_x_loop:
                ...
                {
#pragma HLS UNROLL
                    kernel[out_channel][in_channel].data[y][x] = data_type(weights[x + y * 3 + in_channel * 9 + out_channel * N * 9]);
                }
            }
        }
    }
    ...
    for (int out_channel = 0; out_channel < M; out_channel++)
    {
        for (int in_channel = 0; in_channel < N; in_channel++)
        {
            for (int i = 0; i < width * height; i++)
            {
                ...
                data_type conv = data.mul_v2(kernel[out_channel][in_channel]);

                packet_type out_packet;
                out_packet.data = float(conv);
                out_packet.keep = -1;
                out_packet.strb = -1;
                if (i == width * height - 1 && channel == channels - 1)
